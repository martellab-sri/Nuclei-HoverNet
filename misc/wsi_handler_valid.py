...
class FileHandler(object):
    ...
    def get_dimensions(self, start, end, read_mag=None, read_mpp=None):
        """Will be in X, Y."""
        ...
        #dimensions = self.metadata["base_shape"] * scale
        dimensions = (end -start) * scale

        return dimensions.astype(np.int32)
...
class OpenSlideHandler(FileHandler):
    ...
    def get_valid_img(self, start=(0,0), end=(1,1), read_mag=None, read_mpp=None):
        """Only use `read_mag` or `read_mpp`, not both, prioritize `read_mpp`.
        ...
        wsi_img = self.file_ptr.read_region(start, read_lv, read_size)
        wsi_img = np.array(wsi_img)[..., :3]  # remove alpha channel
        if scale_factor is not None:
            # now rescale then return
            if scale_factor > 1.0:
