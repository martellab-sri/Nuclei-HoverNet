...
def _get_patch_top_left_info(img_shape, input_size, output_size):
    ...
    """
    in_out_diff = input_size - output_size #190 = 270- 80
    nr_step = np.floor((img_shape - in_out_diff) / output_size) + 1 #(358,397)
    last_output_coord = (in_out_diff // 2) + (nr_step) * output_size #(28735,31855)
...
def _get_chunk_patch_info(img_shape, chunk_input_shape, patch_input_shape, patch_output_shape):
    ...
    patch_diff_shape = patch_input_shape - patch_output_shape #270-80=190

    chunk_output_shape = chunk_input_shape - patch_diff_shape #9810 = 10000-190
    chunk_output_shape = round_to_multiple(
        chunk_output_shape, patch_output_shape
    ).astype(np.int64) #9760
    chunk_input_shape = (chunk_output_shape + patch_diff_shape).astype(np.int64) #9950 = 9760+190
    # cut it into 80*80 patches, the top left cords, step = 80
...
def _post_proc_para_wrapper(pred_map_mmap_path, tile_info, func, func_kwargs):
    # fun=   self.post_proc_func = models.hovernet.post_proc, process()
...
def _assemble_and_flush(wsi_pred_map_mmap_path, chunk_info, patch_output_list):
    ...
    # wsi_pred_map_ptr[
    #     chunk_info[1][0][0] : chunk_info[1][1][0],
    #     chunk_info[1][0][1] : chunk_info[1][1][1],] = chunk_pred_map
    # np.save(wsi_pred_map_mmap_path, wsi_pred_map_ptr)
...
class InferManager(base.InferManager):
    ...
    def __get_raw_prediction(self, chunk_info_list, patch_info_list):
        ...
        #for idx in range(0, 1):
            ...
            if chunk_patch_info_list.shape[0] == 0:
                print('chunk_patch_info_list.shape[0] is 0 ')
            ...
            proc_pool.apply_async(_assemble_and_flush,args=(wsi_pred_map_mmap_path, chunk_info, patch_output_list),)
            #pdb.set_trace()
            #_assemble_and_flush(wsi_pred_map_mmap_path, chunk_info, patch_output_list)
        ...
        for idx in list(range(tile_info_list.shape[0])):
            ...
            if proc_pool is not None:
                proc_future = proc_pool.submit(
                    ...
                    tile_info,
                    self.post_proc_func, ####  self.post_proc_func = models.hovernet.post_proc, process()
                    func_kwargs,
                )
                ...
                callback(results) #post_proc_normal_tile_callback()
        #pdb.set_trace()
        if proc_pool is not None:
            silent_crash = False
    ...
    def process_single_file(self, msk_idx, wsi_path, msk_path, output_dir):
        ...
        #self.wsi_proc_shape = self.wsi_handler.get_dimensions(start,end,self.proc_mag)
        ...
        if msk_path is not None and os.path.isfile(msk_path):
            ...
            def simple_get_mask(msk_idx):
                scaled_wsi_mag = 1.25  # ! hard coded max is 40X
                wsi_thumb_rgb = self.wsi_handler.get_full_img(read_mag=scaled_wsi_mag)
                
                h,w = wsi_thumb_rgb.shape[0:2]
                left = int((w*msk_idx)/self.wsi_proc_shape[1])
                print("left: ",left, "w: ",w)
                ...
                _, mask = cv2.threshold(gray, 10, 225, cv2.THRESH_OTSU)
                mask[:,0:left] = 255
                #cv2.imwrite('/labs3/amartel_data3/tingxiao/hover_net/dataset/sample_wsis/th.png', mask)
                mask = morphology.remove_small_objects(mask == 0, min_size=10 * 10, connectivity=2)
                mask = morphology.remove_small_holes(mask, area_threshold=128 * 128)
                mask = morphology.binary_dilation(mask, morphology.disk(16))               
                return mask

            self.wsi_mask = np.array(simple_get_mask(msk_idx) > 0, dtype=np.uint8)
        ...
        if self.save_thumb:
            wsi_thumb_rgb = self.wsi_handler.get_full_img(read_mag=1.0)
            cv2.imwrite(
        ...
        def post_proc_normal_tile_callback(args):
            ...
            #print("##############@: ", np.unique(self.wsi_inst_map))
    ...
    def make_mask_by_index(self, msk_idx, wsi_path, msk_path, output_dir):
        path_obj = pathlib.Path(wsi_path)
        wsi_ext = path_obj.suffix
        wsi_name = path_obj.stem
        self.wsi_handler = get_file_handler(wsi_path, backend=wsi_ext)
    ...
    def process_wsi_list(self, run_args):
        ...
        df = pd.read_excel("/home/tingxiao/code/hover_net/case_mask_mds.xlsx")

        wsi_path_list = glob.glob(self.input_dir + "/*")
        wsi_path_list.sort()  # ensure ordering
        for wsi_path in wsi_path_list[:]:
            wsi_base_name = pathlib.Path(wsi_path).stem
            print(wsi_path,wsi_base_name)
            case = wsi_base_name.split(" ")[0]  ### AML is 1, MDS is 0
            print(case, df.loc[df["case"]==case,"start"])
            start = df.loc[df["case"]==case,"start"].tolist()[0]
            #pdb.set_trace()
            #print(start)
            ...
            try:
                ...
                #self.make_mask_by_index(start, wsi_path, msk_path, self.output_dir)
                
                self.process_single_file(start, wsi_path, msk_path, self.output_dir)
                log_info("Finish")
            except:
