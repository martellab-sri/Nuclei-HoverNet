...
def _post_process_patches(
    ...
    # models.hovernet.post_proc process
    #pred_map.shape = (512, 512, 5), tp, np, hv=2, in 1

    #pred_inst, inst_info_dict, pred_intens = post_proc_func(pred_map, **post_proc_kwargs)
    ...
    #return image_info["name"], pred_map, pred_inst, pred_intens, inst_info_dict, overlaid_img
...
class InferManager(base.InferManager):
    ...
    def process_file_list(self, run_args):
        ...
        file_path_list = glob.glob(patterning("%s/*.tif" % self.input_dir))
        ...
        def proc_callback(results):
            ...
            """
            #img_name, pred_map, pred_inst, pred_intens, inst_info_dict, overlaid_img = results
