...
def bce_loss(true, pred):
    '''
    mask = np.zeros(true.shape, dtype=np.float32)
    mask[true.cpu().detach().numpy() >= 0] = 1
    pred = pred*torch.from_numpy(mask).cuda()
    '''
    assert (pred <= 1.).all() 
    assert (pred >= 0.).all()
    assert (true <= 1.).all()
    assert (true >= 0.).all()
    #mask = torch.zeros(true.shape)
    mask = np.zeros(true.shape,dtype=np.float32)
    mask[true.cpu().detach().numpy() > 0] = 1
    #pred = pred*mask
    pred = pred*torch.from_numpy(mask).cuda()
    #print(type(pred),type(true))
    loss = torch.nn.functional.binary_cross_entropy(pred, true)
    return loss
