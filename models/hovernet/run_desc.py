...
from .utils import crop_to_shape, dice_loss, mse_loss, msge_loss, xentropy_loss, mse_loss_1, smooth_l1, bce_loss
...
def train_step(batch_data, run_info):
    ...
    #print(true_in.shape,type(true_in))
    #true_in = min_max_norm(true_in)
    #true_in = true_in.to("cuda").type(torch.float32)
    ...
    #torch.Size([4, 80, 80, 2]) torch.Size([4, 80, 80, 6]) torch.Size([4, 80, 80, 2])
    #print("###:",pred_dict["np"].shape,pred_dict["tp"].shape,pred_dict["hv"].shape)
    pred_dict["np"] = F.softmax(pred_dict["np"], dim=-1)
    #pred_dict["in"] = torch.sigmoid(pred_dict["tp"][...,-1])  # (pred_dict["tp"][...,-1], dim=-1)
    #print("###:",pred_dict["in"].shape, pred_dict["np"].shape) #[4, 80, 80], [4, 80, 80, 2]
    if model.module.nr_types is not None:
        pred_dict["tp"] = F.softmax(pred_dict["tp"][...,0:5], dim=-1)
        #print("###:",pred_dict["tp"].shape) #torch.Size([4, 80, 80, 5])
    if intensity:
        m = torch.nn.ReLU()
        pred_dict["in"] = m(pred_dict["in"][...,0])
        #pred_dict["in"] = m(pred_dict["tp"][...,-1])
        #print(pred_dict["in"].shape)
...
def infer_step(batch_data, model):
    ...
    with torch.no_grad():  # dont compute gradient
        ...
        if "in" in pred_dict:
            #pred_dict["in"] = torch.sigmoid(pred_dict["in"])
            m = torch.nn.ReLU()
            pred_dict["in"] = m(pred_dict["in"])
