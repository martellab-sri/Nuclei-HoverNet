...
class __Kumar_stain(__AbstractDataset):
    """Defines the Kumar dataset as originally introduced in:
    ...
    def load_ann(self, path, with_type=False,flag='blue'):
        # assumes that ann is HxW
        ann_inst = sio.loadmat(path)["inst_map"]
        if with_type:
            ann_type = ann_inst
...
class __CoNSeP(__AbstractDataset):
    ...
    def load_ann(self, path, with_type=False):
        ...
        if with_type:
            ...
            ann_type[(ann_type == 3) | (ann_type == 4)] = 2
...
class __Aml(__AbstractDataset):
    """define our dataset for Acute myeloied lukimia with four or two classes

    """
    def load_img(self, path):
        return cv2.cvtColor(cv2.imread(path), cv2.COLOR_BGR2RGB)

    def load_ann(self, path, with_type=False, nr_type=5):
        # assumes that ann is HxW
        ann_inst = sio.loadmat(path)["inst_map"]
        ann_intense = sio.loadmat(path)["intens_map"]
        if with_type:
            ann_type = sio.loadmat(path)["type_map"]
            if nr_type==3:
            ...
            # If own dataset is used, then the below may need to be modified
                ...
                pass
            ann = np.dstack([ann_inst, ann_type, ann_intense])
            #ann = np.dstack([ann_inst, ann_type])
            ann = ann.astype("int32")
        else:
            ann = np.expand_dims(ann_inst, -1)
...
def get_dataset(name):
    ...
    name_dict = {
        ...
        "kumar-stain": lambda: __Kumar_stain(),
        "aml":lambda: __Aml(),
    }
